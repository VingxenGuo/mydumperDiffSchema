...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP PROCEDURE IF EXISTS `updateTrialPlayerWallet`;
CREATE DEFINER=`root`@`%` PROCEDURE `updateTrialPlayerWallet`(
    IN `in_name` varchar(190),
    IN `in_money` bigint(20),
    IN `in_action` tinyint(1),
    IN `in_currency` varchar(20),
    IN `in_memo` LONGTEXT
)
BEGIN
    DECLARE balance bigint(20); 
    ...
    SELECT money into balance FROM `trial_players` WHERE `name` = in_name AND `currency` = in_currency LIMIT 1 FOR UPDATE; 

    IF balance is null THEN
        IF in_action = 0 THEN  #钱包不存在且上分的话就创建钱包
            INSERT INTO `trial_players` (`name`, `currency`, `money`, `updateDate`) VALUES (in_name, in_currency, in_money, now()); 
            SET updateMoney = in_money; 
        ELSE
            SET errCode = 1001;  #钱包不存在
        END IF; 
    ...
    ELSE
        ...
        #更新钱包
        UPDATE `trial_players` SET money = updateMoney,updateDate=now() WHERE `name`= in_name AND `currency` = in_currency; 
    END IF; 
