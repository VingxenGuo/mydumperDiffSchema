...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP PROCEDURE IF EXISTS `sp_Statis_GetUsers_02`;
CREATE DEFINER=`root`@`%` PROCEDURE `sp_Statis_GetUsers_02`(
...
BEGIN
  ...
  WHILE @i <= @days DO
    ...
    -- TODO: 新增、變更指令
    /*
    REPLACE INTO KYStatis.statisusers(
            StatisDate
          , ChannelId)
     SELECT @curDate AS StatisDate, ChannelID 
       FROM KYDB_NEW.Sys_ProxyAccount; 
    */
    ...
    SET v_sql = CONCAT(
        'UPDATE KYStatis.statisusers AS a
          INNER JOIN (
            ...
            SET a.Y_NonLoginUsers = b.Y_NonLoginUsers
          WHERE StatisDate = ''', @curDate,''' AND a.ChannelID = b.ChannelId'
    ); 
    SET @v_sql = v_sql; 
                  ...
                  FROM (
                       SELECT agent, account
                        ...
                        WHERE createdate >= ''', @curDate,''' 
                          AND createdate <= ''', @curDate,' 23:59:59.998''
                       ) AS m
                 INNER JOIN (
                       SELECT ChannelId, Accounts
                         FROM ', v_tblName_login_detail,'
                    ...
                    ON m.account = login.Accounts
                   AND m.agent = login.ChannelId
                 GROUP BY m.agent
                ) AS T ON a.ChannelID = T.agent
            SET a.dayNewLogin = T.dayNewLogin 
          WHERE StatisDate = ''', @curDate,''''); 
    ...
    UPDATE KYStatis.statisusers AS a
     INNER JOIN (
       ...
       SET a.Y_RegUsers = b.Y_RegUsers
     WHERE StatisDate = @curDate 
       AND a.ChannelID = b.agent; 
    -- */
    ...
    SET v_sql = CONCAT(
        'UPDATE KYStatis.statisusers AS c
          INNER JOIN (
                SELECT a.ChannelId, COUNT(1) AS Y_NewLoginUsers
                  FROM (
                       SELECT Accounts, ChannelId
                  ...
                  LEFT JOIN KYStatisLogin.statis_login_hall_history AS b 
                    ON a.Accounts = b.Accounts
                 WHERE ISNULL(b.Accounts)
                GROUP BY a.ChannelId
                ) AS d
            SET c.Y_NewLoginUsers = d.Y_NewLoginUsers 
          WHERE StatisDate = ''',@curDate,''' 
            AND c.ChannelID = d.ChannelID'
        ); 
