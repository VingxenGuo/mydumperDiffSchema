...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP PROCEDURE IF EXISTS `sp_insert_Sys_ProxyGameInfo`;
CREATE DEFINER=`root`@`%` PROCEDURE `sp_insert_Sys_ProxyGameInfo`(IN v_GameID INT(11))
BEGIN

    INSERT IGNORE into KYDB_NEW.Sys_ProxyGameInfo
    SELECT a.GameId, b.id, 1, 0, 0, NULL 
    FROM KYDB_NEW.GameInfo a
    CROSS JOIN KYDB_NEW.agent b ON a.GameID = v_GameID 
    LEFT JOIN KYDB_NEW.Sys_ProxyGameInfo c ON a.GameId = c.GameId AND b.id = c.ChannelID
