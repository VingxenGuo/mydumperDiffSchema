...
BEGIN
	...
	SET v_sql = CONCAT("INSERT INTO fb.gameRecordInfo SELECT id, 'CNY', 'ZH_CN' FROM ", v_dist_db,'.', v_dist_table); 
	SET @v_sql = v_sql;	

	TRUNCATE TABLE fb.gameRecordInfo; 
...
SELECT
    ...
    CASE WHEN walletType = 2 THEN
		CASE WHEN isAutoPay = 0 THEN 1 ELSE 2 END
	ELSE
		0
	END AS payType,
	...
	CASE WHEN fbStatus = 1 THEN 1 ELSE 0 END AS fbStatus,
	sportCashout AS sportCashout, 
	fb_sportAccountingFor AS fb_sportAccountingFor, 
	sportRoute AS sportRoute, 
	sportDomainStatus AS sportDomainStatus, 
	sportDomain AS sportDomain, 
	sportDomainPC AS sportDomainPC	
FROM
    Sys_ProxyAccount
...
BEGIN
	...
	SET v_base_sql = "SELECT
        ...
        old.companyPWD,
        old.companyname,
        LOWER(old.abbreviation) abbreviation,
        old.whiteip,
        old.mark,
        old.updatetime,
        old.status,
...
BEGIN
    ...
    SET v_base_sql = 'select old.GameID, old.ChannelID, old.GameOrderBy, '; 
    SET v_base_sql = CONCAT(v_base_sql, 'CASE WHEN old.GameStatus IN (2, 4) THEN 1 ELSE old.GameStatus END AS GameStatus, '); 
    SET v_base_sql = CONCAT(v_base_sql, 'old.ShowLabel, old.OutLink from '); 
...
BEGIN
	DECLARE done INT DEFAULT FALSE; 
    DECLARE table_names VARCHAR(255); 
	DECLARE v_table_names VARCHAR(255); 
	DECLARE v_sql LONGTEXT; 
	DECLARE v_base_sql LONGTEXT; 
	DECLARE v_src_db VARCHAR(255); 
	DECLARE v_dist_db VARCHAR(255); 
    ...
    read_loop: LOOP
		...
		IF (table_names LIKE 'statis_fb%') THEN
			SET v_table_names = REPLACE(table_names, 'statis_fb', 'statis_KYSport2'); 
		ELSE
			SET v_table_names = table_names; 
		END IF; 

		set @v_drpoSql = CONCAT('DROP TABLE IF EXISTS ', v_dist_db,'.', v_table_names, ';'); 
		PREPARE stmt FROM @v_drpoSql; 
		EXECUTE stmt; 
		DEALLOCATE PREPARE stmt; 
		
		call KYStatisUsers.sp_createStatisticsUsersTable(v_table_names, 0); 
		...
		SET v_sql = CONCAT('INSERT INTO ', v_dist_db,'.', v_table_names, ' ', v_sql); 
		SET @v_sql = v_sql; 
...
BEGIN
    ...
    read_loop: LOOP
		...
		call KYStatisUsers_EST.sp_createStatisticsUsersTable(v_table_names, 0); 

		SET v_sql = CONCAT('select old.StatisDate, old.Account, old.ChannelID, old.LineCode, 0 Allbet, SUM(old.CellScore) CellScore, SUM(old.WinGold) WinGold, SUM(old.LostGold) LostGold, SUM(old.Revenue) Revenue, SUM(old.WinNum) WinNum, SUM(old.LostNum) LostNum, \'CNY\' currency, 1 exchangeRate, old.CreateTime, old.UpdateTime from ', v_src_db,'.', table_names,' old GROUP BY old.StatisDate, old.Account, old.LineCode'); 
		SET v_sql = CONCAT('INSERT IGNORE INTO ', v_dist_db,'.', v_table_names, ' ', v_sql); 
		SET @v_sql = v_sql; 
