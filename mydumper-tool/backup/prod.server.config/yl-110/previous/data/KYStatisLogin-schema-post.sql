...
BEGIN
    DECLARE v_createSql varchar(1000); 
    DECLARE v_createSql0 varchar(1000); 
    DECLARE v_createSql1 varchar(1000); 
    DECLARE v_createSql2 varchar(1000); 
    DECLARE v_createSql3 varchar(1000); 
    DECLARE v_createSql4 varchar(1000); 
    DECLARE v_createSql5 varchar(1000); 
    DECLARE v_createSql6 varchar(1000); 
    ...
    ELSEIF in_type = 2 or in_type = 4 THEN
        SET v_createSql = v_createSql2; 
    ELSEIF in_type = 3 or in_type = 6 THEN
...
CREATE DEFINER=`root`@`%` PROCEDURE `sp_statisLoginData`(
	IN `in_StatisDate` date
)
BEGIN
	DECLARE v_tblName VARCHAR(100); 
	DECLARE v_orderTblName VARCHAR(100); 
	DECLARE v_accountsTblName VARCHAR(100); 
	DECLARE v_date VARCHAR(6); 
	DECLARE v_sql LONGTEXT; 
	SET @timediff = NOW(); 
	...
	END IF; 
	IF EXISTS(SELECT * FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'orders_record' AND TABLE_NAME = 'agent_orders') THEN
		SET v_sql = CONCAT('REPLACE INTO ',v_tblName,'(StatisDate,ChannelID,Accounts)
						SELECT DISTINCT ''', in_statisdate,''' AS StatisDate, agentOrders.ChannelID, agentOrders.CreateUser 
						FROM orders_record.agent_orders AS agentOrders
						INNER JOIN game_api.accounts AS gaa
						ON agentOrders.CreateUser = gaa.account
						WHERE agentOrders.OrderTime >= ''', in_statisdate,''' AND agentOrders.OrderTime <= ''', in_statisdate,' 23:59:59.998'' AND agentOrders.OrderType =2 AND agentOrders.OrderStatus = 0'); 
		SET @v_sql = v_sql; 
		prepare stmt FROM @v_sql; 
		EXECUTE stmt; 
		DEALLOCATE prepare stmt; 
...
CREATE DEFINER=`root`@`%` EVENT `event_1_00_sp_statisKeepLoginUsers` ON SCHEDULE EVERY 1 DAY STARTS '2021-07-25 03:00:00' ON COMPLETION NOT PRESERVE ENABLE DO call sp_statisKeepLoginUsers(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_sp_statis_users_iplocal_00_40` ON SCHEDULE EVERY 1 DAY STARTS '2021-09-13 02:40:00' ON COMPLETION NOT PRESERVE ENABLE DO call sp_statis_users_iplocal(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_statisKeepGameUsers_EveryDay_01_30` ON SCHEDULE EVERY 1 DAY STARTS '2021-10-25 03:30:00' ON COMPLETION NOT PRESERVE ENABLE DO call KYStatisLogin.sp_statisKeepGameUsers_EveryDay(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_statisKeepGameUsers_EveryDay_01_40_-2` ON SCHEDULE EVERY 1 DAY STARTS '2021-10-25 03:40:00' ON COMPLETION NOT PRESERVE ENABLE DO call KYStatisLogin.sp_statisKeepGameUsers_EveryDay(DATE_ADD(CURDATE(),INTERVAL -2 day));
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_statisKeepLoginUsersValid_02:30` ON SCHEDULE EVERY 1 DAY STARTS '2021-01-01 03:30:00' ON COMPLETION NOT PRESERVE ENABLE DO call sp_statisKeepLoginUsersValid(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_StatisLocationGameDayData_02:40` ON SCHEDULE EVERY 1 DAY STARTS '2023-01-01 02:40:00' ON COMPLETION NOT PRESERVE ENABLE DO call `KYStatisLogin`.`sp_StatisLocationGameDayData`(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_statisLoginData` ON SCHEDULE EVERY 1 DAY STARTS '2021-07-18 02:05:00' ON COMPLETION NOT PRESERVE ENABLE DO call sp_statisLoginData(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_statis_users_visits` ON SCHEDULE EVERY 1 DAY STARTS '2021-09-13 02:10:00' ON COMPLETION NOT PRESERVE DISABLE DO call sp_statis_users_visits(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_statis_validaccounts_02_00` ON SCHEDULE EVERY 1 DAY STARTS '2021-01-01 03:00:00' ON COMPLETION NOT PRESERVE ENABLE DO call sp_statis_validaccounts(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
