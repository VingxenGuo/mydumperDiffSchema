...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP PROCEDURE IF EXISTS `createKYStatisChannelJump`;
CREATE DEFINER=`root`@`%` PROCEDURE `createKYStatisChannelJump`()
BEGIN
	DECLARE v_create_sql LONGTEXT; 
	DECLARE v_dist_db VARCHAR(255); 
	DECLARE v_dist_table VARCHAR(255); 
	DECLARE v_month VARCHAR(6); 
    INSERT INTO Transfer.process_record (`sp_name`) VALUES ('createKYStatisChannelJump') ON DUPLICATE KEY UPDATE `status`= 0; 

	SET v_month = DATE_FORMAT(NOW(), '%Y%m'); 
	SET v_dist_db = 'KYStatis'; 
	SET v_dist_table = CONCAT('channel_jump_', v_month); 
    ...
    INSERT INTO Transfer.process_record (`sp_name`) VALUES ('createKYStatisChannelJump') ON DUPLICATE KEY UPDATE `status`= 1; 
END;
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP PROCEDURE IF EXISTS `createKYStatisUserRouter`;
CREATE DEFINER=`root`@`%` PROCEDURE `createKYStatisUserRouter`()
BEGIN
	DECLARE v_create_sql LONGTEXT; 
	DECLARE v_dist_db VARCHAR(255); 
	DECLARE v_dist_table VARCHAR(255); 
	DECLARE v_month VARCHAR(6); 
    INSERT INTO Transfer.process_record (`sp_name`) VALUES ('createKYStatisUserRouter') ON DUPLICATE KEY UPDATE `status`= 0; 

	SET v_month = DATE_FORMAT(NOW(), '%Y%m'); 
	SET v_dist_db = 'KYStatis'; 
	SET v_dist_table = CONCAT('user_router_', v_month); 
    ...
    INSERT INTO Transfer.process_record (`sp_name`) VALUES ('createKYStatisUserRouter') ON DUPLICATE KEY UPDATE `status`= 1; 
END;
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
...
BEGIN
	...
	SET v_base_sql = 'select old.*, null powergamelist from '; 
