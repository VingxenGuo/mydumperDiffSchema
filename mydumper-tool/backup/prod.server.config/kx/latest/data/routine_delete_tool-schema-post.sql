...
-- 		select partition_name into partname from information_schema.partitions where table_schema = detect_database and table_name = detect_table; 
...
funcbody: begin
    ...
    for_not_game_record_loop: loop
        ...
        if r_exec_table regexp '[a-z]+[0-9]{8}$' then 
			...
			select count(*) into db_exist_routine_type_flag from tool.routine_delete_table where exec_database = r_exec_database and exec_table = r_exec_table and routine_type = 'd'; 
		...
		elseif r_exec_table regexp '[a-z]+[0-9]{6}$' then
            ...
            set r_exec_table = substring(r_exec_table, 1, length(r_exec_table)-8); 
            # 判斷該database的時間格式，是否已存在於routine_delete_table
			SELECT COUNT(*) INTO db_exist_routine_type_flag FROM tool.routine_delete_table where exec_database = r_exec_database and exec_table = r_exec_table and  routine_type = 'm'; 
			if db_exist_routine_type_flag = 0 then
				# insert data into depand on routine_type
				INSERT INTO `tool`.`routine_delete_table` ( `exec_database`, `exec_table`, `routine_type`, `delete_type`, `time_interval`, `is_enabled`) values (r_exec_database, r_exec_table, 				routine_type, 'drop_table', time_interval, 1); 
				iterate for_not_game_record_loop; 
		...
		elseif r_exec_table regexp '[a-z]+[0-9]{4}$' then
            ...
            set r_exec_table = substring(r_exec_table, 1, length(r_exec_table)-8); 
            # 判斷該database的時間格式，是否已存在於routine_delete_table
			select count(*) into db_exist_routine_type_flag from tool.routine_delete_table where exec_database = r_exec_database and exec_table = r_exec_table and routine_type = 'y'; 
            if db_exist_routine_type_flag = 0 then
				# insert data into depand on routine_type
				INSERT INTO `tool`.`routine_delete_table` ( `exec_database`, `exec_table`, `routine_type`, `delete_type`,  `time_interval`, `is_enabled`) values (r_exec_database, r_exec_table, 				routine_type, 'drop_table', time_interval, 1); 
				iterate for_not_game_record_loop; 
...
BEGIN
    ...
    DECLARE cur2_exec_database VARCHAR(128); 
    DECLARE cur2_exec_table VARCHAR(128); 
