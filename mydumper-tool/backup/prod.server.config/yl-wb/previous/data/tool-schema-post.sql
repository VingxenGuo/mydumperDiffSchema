...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP PROCEDURE IF EXISTS `drop_part`;
CREATE DEFINER=`root`@`%` PROCEDURE `drop_part`()
begin
	declare cursor_done int(1); 
    declare cur_schema_name varchar(64); 
    declare cur_table_name varchar(64); 
    declare cur_partition_name varchar(64); 
	declare cur cursor for select table_schema, table_name from information_schema.partitions where partition_name is not null group by table_schema, table_name; 
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET cursor_done = 1; 
    ...
    drop_part:loop
        ...
        fetch cur into cur_schema_name, cur_table_name; 
			if cursor_done = 1 then
				leave drop_part; 
			end if; 
			
			set @exec_sql = concat('alter table ', cur_schema_name, '.', cur_table_name, ' remove partitioning;\n', @exec_sql); 
...
CREATE DEFINER=`root`@`%` PROCEDURE `remove_partition`()
BODY:BEGIN
	DECLARE result_str TEXT DEFAULT ''; 
    DECLARE r_exec_sql TEXT DEFAULT ''; 
    DECLARE cursor_done BOOL DEFAULT FALSE; 
    DECLARE cursor1 CURSOR FOR SELECT CONCAT('ALTER TABLE ', TABLE_SCHEMA, '.', TABLE_NAME, ' REMOVE PARTITIONING;', "\n") `exec_sql` FROM information_schema.partitions WHERE PARTITION_NAME IS NOT NULL GROUP BY TABLE_SCHEMA, TABLE_NAME; 
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET cursor_done = TRUE; 
    
    OPEN cursor1; 
    PROCESS_LOOP: LOOP
		FETCH cursor1 INTO r_exec_sql; 
        IF cursor_done = TRUE THEN
			LEAVE PROCESS_LOOP; 
		END IF; 
        SET result_str = CONCAT(result_str, r_exec_sql); 
	END LOOP PROCESS_LOOP; 
    SELECT result_str; 
END BODY;
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `auto_delete_process` ON SCHEDULE EVERY 1 DAY STARTS '2023-04-25 04:00:00' ON COMPLETION NOT PRESERVE DISABLE ON SLAVE DO CALL tool.delete_process();
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
