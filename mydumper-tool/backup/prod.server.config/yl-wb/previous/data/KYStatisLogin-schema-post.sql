...
CREATE DEFINER=`root`@`%` PROCEDURE `sp_statisKeepLoginUsersValid`(IN `in_statisdate` date)
BEGIN
	DECLARE v_tblName varchar(100); 
	DECLARE v_orderTblName varchar(100); 
	DECLARE v_date varchar(6); 
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_statis_users_visits` ON SCHEDULE EVERY 1 DAY STARTS '2023-06-03 21:19:35' ON COMPLETION NOT PRESERVE DISABLE ON SLAVE DO call sp_statis_users_visits(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
