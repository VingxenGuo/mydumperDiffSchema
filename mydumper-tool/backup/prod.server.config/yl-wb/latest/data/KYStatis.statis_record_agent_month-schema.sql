...
CREATE TABLE `statis_record_agent_month` (
  ...
  `ActiveUsers_reguser` int(11) DEFAULT '0' COMMENT '新增月注册活跃',
  `currency` varchar(20) NOT NULL COMMENT '幣別',
  `exchangeRate` decimal(20,5) NOT NULL COMMENT '汇率',
  PRIMARY KEY (`ChannelID`,`StatisDate`,`currency`) USING BTREE,
  KEY `IDX_SD` (`StatisDate`) USING BTREE
