...
CREATE DEFINER=`root`@`%` PROCEDURE `sp_memberBetDetail`(IN `accounts` varchar(190), IN `linecode` varchar(190), IN `beginDate` date, IN `endDate` date, IN `channelId` int)
BEGIN
...
BEGIN
    ...
    SELECT ''', in_StatisDate,''' AS StatisDate,ChannelID,SUM(WinGold) AS WinGold,SUM(LostGold) AS LostGold,SUM(CellScore) AS CellScore,SUM(Revenue) AS Revenue,SUM(WinNum) AS WinNum,SUM(LostNum) AS LostNum,count(distinct A.Account) AS ActiveUsers, A.currency, B.exchangeRate 
    FROM ',v_dbName,'.statis_allgames',v_date,'_users AS A
    LEFT JOIN game_manage.rp_currency AS B ON A.currency = B.currency
	LEFT JOIN KYDB_NEW.accounts c on A.Account =c.account 
    WHERE StatisDate = ''',in_StatisDate,''' 
	AND  c.createdate>=''',in_StatisDate,''' AND c.createdate<=''',in_StatisDate,' 23:59:59''
    GROUP BY ChannelID, currency');  
