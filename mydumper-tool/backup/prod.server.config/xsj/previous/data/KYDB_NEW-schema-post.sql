...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP PROCEDURE IF EXISTS `sp_gameRecordSort0220`;
CREATE DEFINER=`root`@`%` PROCEDURE `sp_gameRecordSort0220`(IN `currency` varchar(50), IN `kindId` int,IN `accounts` varchar(190),IN `linecode` varchar(190),IN `beginDate` date,IN `endDate` date,IN `sortType` int,IN `pageOffset` int,IN `pageLimit` int,IN `channelId` int,IN `isPage` bit,IN `tmpRecordCount` int, IN `ServerId` int, IN `ProxyId` int)
BEGIN
	DECLARE v_sql LONGTEXT; 
	DECLARE v_totalSql LONGTEXT; 
	DECLARE v_sqlWhere LONGTEXT;   
...
-- 組 WHERE
    ...
    SET v_sqlWhere = CONCAT('
        WHERE `StatisDate` >= "',beginDate,'"
        AND `StatisDate` <= "',endDate,'"'); 

    IF channelId > 0 THEN -- 代理商后台

        SET v_sqlWhere = CONCAT(
            v_sqlWhere,
            ' AND ', v_aTable, 'ChannelID = ', channelId); 
    ...
    IF LENGTH(linecode) > 0 THEN
        ...
        SET v_sqlWhere = CONCAT(
            v_sqlWhere,
            ' AND ', v_bTable, 'LineCode = ''', linecode, ''''); 
    ...
    IF ProxyId > 0 THEN
        ...
        SET v_sqlWhere = CONCAT(
            v_sqlWhere,
            ' AND ', v_aTable, 'ChannelID = ', ProxyId); 
    ...
    IF LENGTH(currency) > 0 THEN
        ...
        SET v_sqlWhere = CONCAT(
            v_sqlWhere,
            ' AND ', v_aTable, 'currency = ''', currency, ''''); 
    ...
    ELSE
        ...
        SET v_selectName = CONCAT(
                ...
                (', v_aTable, 'WinGold + ', v_aTable, 'LostGold) * ',
                v_aTable, 'exchangeRate  AS Profit, ',
                v_aTable, 'Revenue *',
                v_aTable, 'exchangeRate AS Revenue,
                (', v_aTable, 'WinNum + ', v_aTable, 'LostNum) AS GameNum '
            ); 
...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP PROCEDURE IF EXISTS `sp_gameRecordSort_test`;
CREATE DEFINER=`root`@`%` PROCEDURE `sp_gameRecordSort_test`(IN `currency` varchar(50), IN `kindId` int,IN `accounts` varchar(190),IN `linecode` varchar(190),IN `beginDate` date,IN `endDate` date,IN `sortType` int,IN `pageOffset` int,IN `pageLimit` int,IN `channelId` int,IN `isPage` bit,IN `tmpRecordCount` int, IN `ServerId` int, IN `ProxyId` int)
BEGIN
	DECLARE v_sql LONGTEXT; 
	DECLARE v_totalSql LONGTEXT; 
	DECLARE v_sqlWhere LONGTEXT;   
...
BEGIN
	...
	IF LENGTH(v_sqlbase) <> 0 THEN		
		...
		DROP TEMPORARY TABLE IF EXISTS all_game_record; 

		CREATE TEMPORARY TABLE all_game_record
		(`GameID` bigint(20),
		`Accounts` varchar(190) ,
		`ServerID` int(11) ,
		`RoomType` int(11) ,
		`KindID` int(11) ,
		`TableID` bigint(20) ,
		...
		`CardValue` varchar(1000) ,
		`OpValue` bigint(20) ,
		`ChannelID` int(11) ,
		`LineCode` varchar(50) ,
		`GameUserNO` varchar(50) ,
		`language` varchar(50) ,
		`currency` varchar(50) ,
		...
		IF isPage = 1 THEN
			...
			SET v_sqlselect = CONCAT('SELECT * FROM all_game_record LIMIT ', pageIndex, ',', pageSize); 

		ELSE

			SET v_sqlselect = CONCAT('SELECT * FROM all_game_record'); 
...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP PROCEDURE IF EXISTS `sp_GetGameRecord5`;
CREATE DEFINER=`root`@`%` PROCEDURE `sp_GetGameRecord5`(OUT `recordTotal` int,IN `gameSQL` text,IN `pageSize` int,IN `pageIndex` int,IN `orderString` varchar(100), IN `isPage` bit)
BEGIN
	DECLARE v_sqlcount LONGTEXT; 
	DECLARE v_sqlselect LONGTEXT; 
	DECLARE v_sqlbase LONGTEXT; 
	...
	IF LENGTH(v_sqlbase) <> 0 THEN		
		...
		SET v_sqlcount = CONCAT('select count(*)  as totalCount, SUM(CellScore) as CellScore, SUM(Profit) as Profit, 
				...
				from ', '(', v_sqlbase, ') as main group by Currency'); 
...
BEGIN
    ...
    IF v_sqlbase IS NOT NULL THEN
        SET v_sqlselect = CONCAT('SELECT SQL_CALC_FOUND_ROWS * FROM ( ',v_sqlbase,' ) detail_record ORDER BY detail_record.GameEndTime DESC '); 
        IF isPage = 1 THEN
            SET v_sqlselect = CONCAT(v_sqlselect,' LIMIT ',pageIndex,',',pageSize); 
        END IF; 
...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP PROCEDURE IF EXISTS `sp_gridViewPager2`;
CREATE DEFINER=`root`@`%` PROCEDURE `sp_gridViewPager2`(OUT `recordTotal` int,IN `viewName` varchar(100),IN `fieldName` varchar(200),IN `keyName` varchar(50),IN `pageSize` int,IN `pageIndex` int,IN `orderString` varchar(100),IN `whereString` varchar(500),IN `tmpRecordCount` int,IN `isPage` bit)
BEGIN
	DECLARE v_sqlcount VARCHAR(4000); 
	DECLARE v_sqlselect VARCHAR(4000); 
...
-- 不選遊戲
        ...
        END IF; 
            ...
            SET v_sql = CONCAT(
                ...
                WHERE acct.agent = ', channelId,'
                AND acct.Account = ''', accounts, '''
                AND acct.LineCode = ''', linecode, '''' ); 
...
CREATE DEFINER=`root`@`%` PROCEDURE `sp_ProxyOrderDetail`(OUT `recordTotal` int,IN `pageSize` int,IN `pageIndex` int,IN `whereString` varchar(500),IN `tmpRecordCount` int,IN `orderType` int,IN `isPage` bit)
BEGIN
	DECLARE v_sqlselect LONGTEXT; 
	DECLARE v_sqlcount LONGTEXT; 
	DECLARE v_sqlbase LONGTEXT; 
	...
	IF LENGTH(whereString) <> 0 THEN
		SET v_sqlplayer = CONCAT('SELECT OrderID, OrderTime, ChannelID, OrderType, OrderStatus, 
										CurScore, AddScore, NewScore, OrderIP, CreateUser,'''' AS OrderObject 
									FROM orders_record.agent_orders FORCE index(index_ordertime) 
									WHERE 1 = 1 ', whereString); 

		IF orderType = -1 THEN
			SET v_sqlbase = CONCAT(v_sqlplayer, ' UNION ALL SELECT OrderID, OrderTime, ChannelID, OrderType, OrderStatus, 
											CurScore, AddScore, NewScore, OrderIP, CreateUser, OrderObject 
		...
		ELSEIF orderType = 0 OR orderType = 1 OR orderType = 4 OR orderType = 5 OR orderType = 6 OR orderType = 7 THEN
			SET v_sqlbase = CONCAT('SELECT OrderID, OrderTime, ChannelID, OrderType, OrderStatus, CurScore, 
											AddScore, NewScore, OrderIP, CreateUser, OrderObject 
										FROM KYDB_NEW.Sys_HT_ProxyOrderDetails FORCE index(index_ordertime) 
										WHERE 1 = 1 ',whereString);	
		ELSE
		...
		IF LENGTH(v_sqlbase) <> 0 THEN
			SET v_sqlselect = CONCAT('SELECT main.*, g.Accounts AS ChannelName, IFNULL(r.Accounts,''system'') AS PChannelName 
										FROM (',v_sqlbase,') AS main 
										LEFT JOIN KYDB_NEW.Sys_ProxyAccount AS g ON main.ChannelId = g.ChannelId 
										LEFT JOIN KYDB_NEW.Sys_ProxyAccount AS r ON g.UID = r.ChannelId 
...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP PROCEDURE IF EXISTS `sp_reLoop`;
CREATE DEFINER=`root`@`%` PROCEDURE `sp_reLoop`(
    IN `in_Date` DATE
)
    COMMENT '重复执行统计sp, null为执行昨天'
BEGIN
    DECLARE inDate DATE; 

    IF in_Date IS NULL THEN
        SET inDate = DATE_ADD(CURDATE(),INTERVAL -1 DAY); 
    ...
    -- KYStatis.event_sp_statis_room_recordtype_data (5/35 * * * *)
    CALL KYStatis.sp_statis_room_recordtype_data(@inDate,@inDate); 

    -- KYStatisUsers.event_00_20_sp_StatisUsersDayData (20 0 * * *) 
    CALL KYStatisUsers.sp_StatisUsersByDayWithoutAcc(@inDate); 

    -- KYStatisUsers.event_sp_UpdateDayrecordVvipUsers_00:30 (30 0 * * *)
    CALL KYStatisUsers.sp_UpdateDayrecordVvipUsers(@inDate); 

    -- KYDB_NEW.event_statisAgentRecordInfo (40 0 * * *)
    CALL KYDB_NEW.sp_statisAgentRecordInfo_everyMonth(@inDate); 

    -- KYDB_NEW.event_sp_StatisGameRoomData_reguser_everyDay_01:30 (30 1 * * *)
    CALL KYDB_NEW.sp_StatisGameRoomData_reguser_everyDay(@inDate); 

    -- KYStatisUsers_EST.event_StatisUsersDayData (30 1 * * *) 
    CALL KYStatisUsers_EST.sp_StatisUsersDayData(@inDate); 

    -- KYDB_NEW.event_sp_StatisGameRoomData_everyDay_2:00 (0 2 * * *)
    CALL KYDB_NEW.sp_StatisGameRoomData_everyDay(@inDate); 

    --  KYStatisLogin.event_sp_statisLoginData (5 2 * * * )
    CALL KYStatisLogin.sp_statisLoginData(@inDate); 

    -- KYDB_NEW.event_sp_statis_agent_month_activeUsers_reguser2:10 (10 2 * * *)
    CALL KYDB_NEW.sp_statis_agent_month_activeUsers_reguser(@inDate); 

    -- KYDB_NEW.event_sp_StatisLinecodeMonitoring_everyDay_2:30 (30 2 * * *)
    CALL KYDB_NEW.sp_StatisLinecodeMonitoring_everyDay(@inDate); 

    -- KYDB_NEW.event_sp_Statis_GetUsers (30 2 * * *)
    CALL KYDB_NEW.sp_Statis_GetUsers(@inDate, @inDate); 

    -- KYStatisUsers.event_sp_StatisUsersData_EveryDay (30 2 * * *)
    CALL KYStatisUsers.sp_StatisUsersData_EveryDay(@inDate); 

    -- KYDB_NEW.event_statisAgentRecordInfo_EST (30 2 * * *)
    CALL KYDB_NEW.sp_statisAgentRecordInfo_EST_everyMonth(@inDate); 

    -- KYDB_NEW.event_sp_statisRecord_month (50 2 * * *)
    CALL KYDB_NEW.sp_statisRecord_month(@inDate); 

    -- KYStatisLogin.event_sp_statisKeepLoginUsersValid_02:30 (30 3 * * *)
    CALL KYStatisLogin.sp_statisKeepLoginUsersValid(@inDate); 

    -- KYStatisUsers.event_06_30_sp_StatisUsersDayData_room (30 6 * * *)
    CALL KYStatisUsers.sp_StatisUsersDayData_room(@inDate); 
...
BEGIN DECLARE v_sql LONGTEXT; 

DECLARE v_whereStr LONGTEXT; 
...
IF endDate IS NOT NULL
    AND endDate <> "" THEN
    SET
        v_whereStr = CONCAT(v_whereStr, " AND a.OrderTime <='", endDate, "'"); 
...
IF channelId IS NOT NULL
    AND channelId <> "" THEN
    SET
        v_whereStr = CONCAT(v_whereStr, " AND a.ChannelID =", channelId); 
...
IF account IS NOT NULL
    AND account <> "" THEN
    SET
        v_whereStr = CONCAT(v_whereStr, " AND a.CreateUser ='", account, "'"); 
...
IF orderType IS NOT NULL
    AND orderType <> "" THEN
    SET
        v_whereStr = CONCAT(v_whereStr, " AND a.OrderType =", orderType); 
...
IF orderStatus IS NOT NULL
    ...
    SET
        v_whereStr = CONCAT(v_whereStr, " AND a.OrderStatus =", orderStatus); 
...
IF orderId IS NOT NULL
    AND orderId <> "" THEN
    SET
        v_whereStr = CONCAT(v_whereStr, " AND a.OrderID ='", orderId, "'"); 

END IF; 

IF gameNo IS NOT NULL
    AND gameNo <> "" THEN
    SET
        v_whereStr = CONCAT(v_whereStr, " AND a.GameNo ='", gameNo, "'"); 
...
ELSE
    SET
		...
		SELECT
    		...
    		a.OrderAction AS orderAction
		FROM
    		orders_record.single_orders a
		INNER JOIN (
    		SELECT
        		CASE
            		WHEN SUBSTRING(a.OrderID, 1, 1) REGEXP \'[0-9]\' THEN
                		SUBSTRING_INDEX(a.OrderID, \'_\', 1)
            		ELSE
                		a.OrderID
    		...
    		GROUP BY
        		...
        		END
    		ORDER BY
        		a.OrderTime DESC, a.id DESC
		) maxIds ON a.id = maxIds.maxID
		ORDER BY
    		OrderTime DESC, a.id DESC'
		); 
...
IF isPage = 1 THEN
    SET
        v_select = CONCAT(v_sql, ' limit ', pageIndex, ',', pageSize); 
ELSE
    SET
        v_select = v_sql; 
...
BEGIN
	...
	SET v_sqlBase = CONCAT('
        ...
        JOIN (
            SELECT ''',v_date,''' AS statisDate, ChannelID, COUNT(DISTINCT Account) userCount
            FROM KYStatisUsers.statis_all_users 
            WHERE CreateTime >= ''',v_date,'-02 00:00:00'' 
            AND CreateTime <= ''',v_date2,'-01 23:59:59'' 
            GROUP BY ChannelID) AS b ON a.ChannelID = b.ChannelID AND a.StatisDate = b.statisDate
...
BEGIN
	...
	IF orderType <> -1 AND orderType <> -2 THEN
		SET v_orderwhere = CONCAT(v_orderwhere,' AND OrderType = ',orderType); 
		SET v_ht_orderwhere = CONCAT(v_ht_orderwhere,' AND OrderType =',orderType); 
	...
	IF channelId > 0 THEN
		...
		SET v_ht_orderwhere = CONCAT(v_ht_orderwhere,' AND ChannelID =',channelId); 
	END IF; 

	IF LENGTH(accounts) > 0 THEN
		SET v_orderwhere = CONCAT(v_orderwhere,' AND CreateUser = ''',accounts,''''); 
		SET v_ht_orderwhere = CONCAT(v_ht_orderwhere,' AND Accounts = ''',accounts,''''); 
	END IF; 
	...
	IF (orderType = -1 OR orderType = 0 OR orderType = 1) AND (orderStatus = -1 OR orderStatus = 0) THEN
		...
		SET v_sqlorder = CONCAT(IF(LENGTH(v_sqlorder) <> 0,CONCAT(v_sqlorder,' UNION ALL '),''),
						' SELECT OrderID, OrderTime, ChannelID, OrderType, 0 AS OrderStatus, CurScore, AddScore, 
								NewScore, OrderIP, Accounts AS account,CreateUser, currency 
							FROM KYDB_NEW.Game_HT_UserOrderDetails 
...
CREATE DEFINER=`root`@`%` EVENT `event_call_sp_cron_partition` ON SCHEDULE EVERY 1 DAY STARTS '2023-01-17 03:00:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL KYDB_NEW.sp_cron_partition();
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_AgentAccessGame` ON SCHEDULE EVERY 1 DAY STARTS '2020-02-22 06:00:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL KYDB_NEW.sp_AgentAccessGame (2, NULL);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_StatisGameRoomData_everyDay_2:00` ON SCHEDULE EVERY 1 DAY STARTS '2018-11-16 02:00:00' ON COMPLETION NOT PRESERVE ENABLE DO call sp_StatisGameRoomData_everyDay(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_StatisGameRoomData_reguser_everyDay_01:30` ON SCHEDULE EVERY 1 DAY STARTS '2018-11-16 01:30:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL sp_StatisGameRoomData_reguser_everyDay(NULL);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_StatisLinecodeMonitoring_everyDay_2:30` ON SCHEDULE EVERY 1 DAY STARTS '2021-11-16 03:30:00' ON COMPLETION NOT PRESERVE ENABLE DO call sp_StatisLinecodeMonitoring_everyDay(NULL);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_statisRecord_month` ON SCHEDULE EVERY 1 DAY STARTS '2021-09-06 02:50:00' ON COMPLETION NOT PRESERVE ENABLE DO call sp_statisRecord_month(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_statis_agent_month_activeUsers_reguser2:10` ON SCHEDULE EVERY 1 MONTH STARTS '2019-01-01 02:10:00' ON COMPLETION NOT PRESERVE ENABLE DO call sp_statis_agent_month_activeUsers_reguser(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_Statis_GetUsers` ON SCHEDULE EVERY 1 DAY STARTS '2021-07-21 02:30:00' ON COMPLETION NOT PRESERVE ENABLE DO call sp_Statis_GetUsers(null,null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_statisAgentRecordInfo` ON SCHEDULE EVERY 1 DAY STARTS '2021-05-16 00:40:00' ON COMPLETION NOT PRESERVE ENABLE DO call KYDB_NEW.sp_statisAgentRecordInfo_everyMonth(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_statisAgentRecordInfo_EST` ON SCHEDULE EVERY 1 DAY STARTS '2023-06-17 14:30:00' ON COMPLETION NOT PRESERVE ENABLE DO call KYDB_NEW.sp_statisAgentRecordInfo_EST_everyMonth(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_statis_agent` ON SCHEDULE EVERY 1 DAY STARTS '2021-05-31 05:00:00' ON COMPLETION NOT PRESERVE ENABLE DO call sp_statis_agent;
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
