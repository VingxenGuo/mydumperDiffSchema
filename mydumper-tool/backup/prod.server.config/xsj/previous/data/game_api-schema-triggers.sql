...
BEGIN
 ...
 SET channelID =  json->>"$.agent";  
 SET orderTime =  NEW.createdate; 
 SET addScore =  json->>"$.addScore"; 
 SET orderIP =  json->>"$.ip"; 
 SET createUser =  json->>"$.account"; 
 SET trx_currency =  NEW.currency; 
 SET trx_addScore =  NEW.money; 


 IF (mode = 11 OR mode = 100) THEN  
    SET orderStatus = 0; 
    SET agentCurScore = json->>"$.agentCurMoney"; 
    
	IF type = 0 THEN
		SET agentOrderType = 2; 
        SET playerOrderType = 6; 
		SET agentNewScore = agentCurScore - addScore ; 
		SET playerCurScore = IFNULL(json->>"$.accountCurMoney",0); 
        SET playerNewScore = playerCurScore + trx_addScore ; 
    ELSE 
		SET agentOrderType = 3; 
        SET playerOrderType = 7; 
        SET agentNewScore = agentCurScore + addScore ; 
		SET playerCurScore = IFNULL(json->>"$.accountCurMoney",trx_addScore); 
        SET playerNewScore = playerCurScore - trx_addScore ; 
    END IF; 
...
ELSEIF (mode = 103 OR mode =104 OR mode =105 OR mode =106 OR mode =107 OR mode =108) THEN   
	
	SET orderStatus = mode - 100; 
    SET agentCurScore = IFNULL(json->>"$.agentCurMoney",0); 
	SET agentNewScore = agentCurScore ; 
