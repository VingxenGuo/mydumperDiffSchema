...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP PROCEDURE IF EXISTS `sp_create_finance_sportdielivery_report`;
CREATE DEFINER=`root`@`%` PROCEDURE `sp_create_finance_sportdielivery_report`(IN `createDate1` date,IN `createDate2` date,IN `channelIds` varchar(4000),IN `pzUID` int,IN `gameID` int)
BEGIN
    declare v_sql longtext; 
    declare v_sql_proxy longtext; 
    declare v_tblName varchar(100); 
    ...
    set v_delSql = concat('delete from finance_manage.finance_sportdielivery_detail WHERE StatisDate >= ''', createDate1,''' and StatisDate <= ''', createDate2,'''and gameID = ',gameID,''); 
    if length(v_channelIdList) > 0 then
        set v_delSql = concat(v_delSql,' and find_in_set(channelID,''', v_channelIdList,''')'); 
    end if; 
    set @v_delSql = v_delSql; 
    ...
    set v_sql = concat('replace into finance_manage.finance_sportdielivery_detail(StatisDate,ChannelID,ChannelName,NickName,AccountingFor,CellScore,Profit,SumProfit,TimeZone,gameID)'); 
    set v_sql = concat(v_sql,'select A.StatisDate, B.ChannelID,B.Accounts,B.NickName,B.sportAccountingFor as AccountingFor,sum(A.CellScore) as CellScore,sum((A.WinGold + A.LostGold)) as Profit,0 as SumProfit,0,gameID
									from KYStatis.statis_record_agent_game A inner join KYDB_NEW.Sys_ProxyAccount B on A.ChannelID = B.ChannelID
									where A.StatisDate >= ''', createDate1,''' and A.StatisDate <= ''', createDate2,''' and A.ChannelID not in (select channelId from finance_manage.special_agent_manage where type in (0,5)) And A.GameID = ',gameID,' group by A.StatisDate, A.ChannelID '); 
    if length(v_channelIdList) > 0 then
        set v_sql = concat(v_sql,' and find_in_set(A.channelId,''', v_channelIdList,''')'); 
    end if; 
    ...
    set v_sql = concat('replace into finance_manage.finance_sportdielivery_detail(StatisDate,ChannelID,ChannelName,NickName,AccountingFor,CellScore,Profit,SumProfit,TimeZone,gameID)'); 
    set v_sql = concat(v_sql,'select A.StatisDate, B.ChannelID,B.Accounts,B.NickName,B.sportAccountingFor as AccountingFor,sum(A.CellScore) as CellScore,sum((A.WinGold + A.LostGold)) as Profit,0 as SumProfit,1,gameID
									from KYStatis.statis_record_agent_game_EST A inner join KYDB_NEW.Sys_ProxyAccount B on A.ChannelID = B.ChannelID
									where A.StatisDate >= ''', createDate1,''' and A.StatisDate <= ''', createDate2,''' and A.ChannelID in (select channelId from finance_manage.special_agent_manage where type in (0,5)) And A.GameID = ',gameID,' group by A.StatisDate, A.ChannelID '); 
    if length(v_channelIdList) > 0 then
        set v_sql = concat(v_sql,' and find_in_set(A.channelId,''', v_channelIdList,''')'); 
    end if; 
    ...
    call KYDB_NEW.sp_statis_agent_top_all_line(pzUID); 
    set v_sql = concat('update finance_manage.finance_sportdielivery_detail main inner join(
										select temp.TopUID,A.StatisDate, sum(Profit) as sumProfit from finance_manage.finance_sportdielivery_detail A inner join KYDB_NEW.TEMP_AGENT_TOP temp on A.ChannelID = temp.ChannelID
										where A.StatisDate >= ''', createDate1,''' and A.StatisDate <= ''', createDate2,'''and A.gameID = ',gameID,''); 
    if length(v_channelIdList) > 0 then
        set v_sql = concat(v_sql,' and find_in_set(A.channelId,''', v_channelIdList,''')'); 
    end if; 
    set v_sql = concat(v_sql,' group by temp.TopUID,A.StatisDate) T set main.SumProfit = T.sumProfit where main.ChannelID = T.TopUID and main.StatisDate = T.StatisDate'); 
    select v_sql; 
    set @v_sql = v_sql; 
    prepare stmt from @v_sql; 
    execute stmt; 
    ...
    set v_sql = concat('delete from finance_manage.finance_sportdielivery_list where statisdate = date_format(''', createDate1,''',''%Y-%m'') and gameID = ',gameID,''); 
    if length(v_channelIdList) > 0 then
        set v_sql = concat(v_sql,' and find_in_set(channelId,''', v_channelIdList,''');'); 
    end if; 
    set @v_sql = v_sql; 
    ...
    set v_sql = concat('replace into finance_manage.finance_sportdielivery_list(StatisDate,ChannelID,ChannelName,NickName,AccountingFor,CellScore,Profit,SumProfit,TimeZone,gameID)
										select date_format(''', createDate1,''',''%Y-%m''),ChannelID,ChannelName,NickName,AccountingFor,sum(CellScore) as CellScore,sum(Profit) as Profit,sum(SumProfit) as SumProfit,TimeZone,gameID
										from finance_manage.finance_sportdielivery_detail where StatisDate >= ''', createDate1,''' and StatisDate <= ''', createDate2,'''and gameID = ',gameID,''); 
    if length(v_channelIdList) > 0 then
        set v_sql = concat(v_sql,' and find_in_set(channelId,''', v_channelIdList,''')'); 
    end if; 
    set v_sql = concat(v_sql,' group by ChannelID,ChannelName,NickName,AccountingFor,TimeZone;'); 
    set @v_sql = v_sql; 
    prepare stmt from @v_sql; 
    execute stmt; 
    deallocate prepare stmt; 
