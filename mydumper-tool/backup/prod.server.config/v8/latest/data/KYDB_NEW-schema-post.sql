...
END IF; 
			...
			IF EXISTS(SELECT * FROM information_schema.TABLES WHERE TABLE_SCHEMA = v_dbName AND TABLE_NAME = CONCAT('statis_', cur_GameParameter, v_date, '_users')) THEN
				SET v_sqlbase = CONCAT(IF(LENGTH(v_sqlbase) > 0,CONCAT(v_sqlbase,' UNION ALL '),''),'SELECT Account, WinGold, LostGold, CellScore, Revenue, WinNum, LostNum, ChannelID, ', cur_Gameid, ' AS KindID, LineCode, currency
...
SELECT in_StatisDate, ChannelID, 0, 0, 0, 0, 0, 0, v_currency, v_exchangeRate FROM Sys_ProxyAccount WHERE UserStatus = 0
                                                                                                      AND ChannelID NOT IN (SELECT ChannelID FROM KYStatis.statis_record_agent_all_BST WHERE statisdate = in_StatisDate); 
...
SELECT in_StatisDate, ChannelID, v_gameId, 0, 0, 0, 0, 0, 0, v_currency, v_exchangeRate FROM Sys_ProxyAccount WHERE UserStatus = 0
                                                                                                                AND ChannelID NOT IN (SELECT ChannelID FROM KYStatis.statis_record_agent_game_BST WHERE statisdate = in_StatisDate); 
...
BEGIN
	...
	IF LENGTH(v_sqlbase) <> 0 THEN
		...
		SELECT in_StatisDate, ChannelID, 0, 0, 0, 0, 0, 0, v_currency, v_exchangeRate FROM Sys_ProxyAccount WHERE ChannelID NOT IN (SELECT ChannelID FROM KYStatis.statis_record_agent_all_EST WHERE statisdate = in_StatisDate); 
		...
		SELECT in_StatisDate, ChannelID, v_gameId, 0, 0, 0, 0, 0, 0, v_currency, v_exchangeRate FROM Sys_ProxyAccount WHERE ChannelID NOT IN (SELECT ChannelID FROM KYStatis.statis_record_agent_game_EST WHERE statisdate = in_StatisDate); 
...
BEGIN
	...
	IF LENGTH(v_sqlbase) <> 0 THEN
		...
		SELECT in_StatisDate, ChannelID, 0, 0, 0, 0, 0, 0, v_currency, v_exchangeRate FROM Sys_ProxyAccount WHERE ChannelID NOT IN (SELECT ChannelID FROM KYStatis.statis_record_agent_all WHERE statisdate = in_StatisDate); 
		...
		SELECT in_StatisDate, ChannelID, v_gameId, 0, 0, 0, 0, 0, 0, v_currency, v_exchangeRate FROM Sys_ProxyAccount WHERE ChannelID NOT IN (SELECT ChannelID FROM KYStatis.statis_record_agent_game WHERE statisdate = in_StatisDate); 
...
CREATE DEFINER=`root`@`%` EVENT `event_call_sp_cron_partition` ON SCHEDULE EVERY 1 DAY STARTS '2023-01-17 03:00:00' ON COMPLETION NOT PRESERVE DISABLE ON SLAVE DO CALL KYDB_NEW.sp_cron_partition();
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
