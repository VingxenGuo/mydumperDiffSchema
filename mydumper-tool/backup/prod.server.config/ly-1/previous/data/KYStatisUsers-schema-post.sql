...
BEGIN
    ...
    DECLARE v_sTime VARCHAR(100); 

    DECLARE v_eTime VARCHAR(100); 
    ...
    SET v_sTime = CONCAT(in_StatisDate, ' 00:00:00 '); 

    SET v_eTime = CONCAT(in_StatisDate, ' 23:59:59 '); 

    SET v_sqlWhere = CONCAT(" WHERE GameEndTime >= '", v_sTime, "' AND GameEndTime <= '", v_eTime, "' "); 
    ...
    read_loop: LOOP
        ...
        IF EXISTS(SELECT * FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'detail_record' AND TABLE_NAME = v_tblname) THEN
            ...
            IF NOT EXISTS(SELECT * FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'detail_record' AND TABLE_NAME = CONCAT(v_tblname, '_users')) THEN

                CALL sp_createStatisticsUsersTable(CONCAT('detail_record.', v_tblname, '_users'),0); 

            END IF; 



            SET @v_sqlselect = CONCAT('REPLACE INTO detail_record.', v_tblname, '_users (StatisDate, Account, ChannelID, LineCode, CellScore, WinGold, LostGold, Revenue, WinNum, LostNum, Currency, exchangeRate)

												SELECT ''', in_StatisDate, ''' AS StatisDate, Accounts, ChannelID, LineCode, SUM(CellScore) CellScore, SUM(CASE WHEN Profit>0 THEN Profit else 0 END) wingold,

													SUM(CASE WHEN Profit < 0 THEN Profit else 0 END) lostgold, SUM(Revenue) Revenue, COUNT(CASE WHEN Profit>=0 THEN Profit END) winNum,

													COUNT(CASE WHEN Profit < 0 THEN Profit END) lostNum, a.Currency, d.exchangeRate
                ...
                ,v_sqlWhere,'
													...
													GROUP BY a.Accounts, a.ChannelID, a.LineCode, a.Currency'); 
            ...
            SET v_sqlbase= CONCAT('SELECT * FROM detail_record.', v_tblname, '_users WHERE StatisDate = ''',  in_StatisDate, ''''); 
    ...
    SET @v_sqlselect = CONCAT(IF(LENGTH(v_sqlbase) > 0,CONCAT('REPLACE INTO KYStatisUsers.all', v_tblname, '_users (StatisDate, Account, ChannelID, LineCode, CellScore, WinGold, LostGold, Revenue, WinNum, LostNum, Currency, exchangeRate)

																	SELECT ''', in_StatisDate,''' AS StatisDate, Account, ChannelID, LineCode, SUM(CellScore) CellScore, SUM(WinGold) WinGold, SUM(LostGold) LostGold, SUM(Revenue) Revenue,

																		SUM(WinNum) WinNum, SUM(LostNum) LostNum, a.Currency, d.exchangeRate
																		...
																		GROUP BY a.StatisDate, a.Account, a.ChannelID, a.LineCode, a.Currency'); 
    ...
    SET @v_sqlselect = CONCAT('UPDATE KYStatisUsers.all', v_tblname, '_users a

									LEFT JOIN KYStatisUsers.statis_month', v_month, '_users b ON a.Account = b.Account AND a.Currency = b.Currency
    ...
    SET @v_sqlselect = CONCAT('INSERT INTO KYStatisUsers.statis_month', v_month, '_users (Account, ChannelID, LineCode, CellScore, WinGold, LostGold, Revenue, WinNum, LostNum, Currency, exchangeRate)

									SELECT a.Account, a.ChannelID, a.LineCode, a.CellScore, a.WinGold, a.LostGold, a.Revenue, a.WinNum, a.LostNum, a.Currency, d.exchangeRate

									FROM KYStatisUsers.all', v_tblname, '_users  a

									LEFT JOIN KYStatisUsers.statis_month', v_month, '_users b ON b.Account = a.Account AND a.Currency = b.Currency
    ...
    SET @v_sqlselect = CONCAT('UPDATE KYStatisUsers.all', v_tblname, '_users a
									...
									LEFT JOIN KYStatisUsers.statis_all_users b ON a.Account = b.Account AND a.Currency = b.Currency
    ...
    SET @v_sqlselect = CONCAT('INSERT INTO KYStatisUsers.statis_all_users (Account, ChannelID, LineCode, CellScore, WinGold, LostGold, Revenue, WinNum, LostNum, Currency, exchangeRate)

									SELECT a.Account, a.ChannelID, a.LineCode, a.CellScore, a.WinGold, a.LostGold, a.Revenue, a.WinNum, a.LostNum, a.Currency, d.exchangeRate

									FROM KYStatisUsers.all', v_tblname, '_users  a

									LEFT JOIN KYStatisUsers.statis_all_users b ON b.Account = a.Account AND b.Currency = a.Currency
    ...
    INSERT INTO KYStatisUsers.event_log_record (sp_name, starttime, ENDtime, exectime,mark) VALUES ('KYStatisUsers.sp_StatisUsersDayData_EveryDay', v_starttime, v_ENDtime, TIMESTAMPDIFF(SECOND,v_starttime,v_ENDtime),''); 
...
CREATE DEFINER=`root`@`%` EVENT `event_sp_UpdateDayrecordVvipUsers_00:30` ON SCHEDULE EVERY 1 DAY STARTS '2023-01-01 00:30:00' ON COMPLETION PRESERVE DISABLE ON SLAVE DO call `KYStatisUsers`.`sp_UpdateDayrecordVvipUsers`(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
