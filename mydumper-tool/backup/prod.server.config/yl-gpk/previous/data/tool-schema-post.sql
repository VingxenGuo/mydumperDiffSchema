...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP PROCEDURE IF EXISTS `reset_v_use_by_kill`;
CREATE DEFINER=`root`@`%` PROCEDURE `reset_v_use_by_kill`()
BEGIN
	DECLARE done INT(1) DEFAULT 0; 
    DECLARE T_ID INT; 
    DECLARE e_sql TEXT; 
	DECLARE cur CURSOR FOR SELECT v.THREAD_ID FROM performance_schema.variables_by_thread v left join information_schema.processlist p on v.THREAD_ID = p.ID WHERE v.VARIABLE_NAME='long_query_time' and v.variable_value = 0 and p.COMMAND = 'sleep' and p.USER = 'nodejs' limit 30; 
	...
	cur_loop: LOOP
        ...
        SET @u_sql = CONCAT('kill ', T_ID, ';'); 
        
        PREPARE stmt FROM @u_sql; 
        EXECUTE stmt; 
        DEALLOCATE PREPARE stmt; 
        
        SET e_sql = CONCAT(e_sql, 'kill ', T_ID, ';\n'); 
