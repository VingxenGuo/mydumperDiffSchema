...
BEGIN
	...
	OPEN cur1; 
		read_loop: LOOP
			...
			CALL KYStatisUsers_BST.sp_StatisUsersGameDayData(in_StatisDate, cur_GameParameter); 
	    	SET v_sqlbase = CONCAT(IF(LENGTH(v_sqlbase) > 0,CONCAT(v_sqlbase,' UNION ALL '),''),'SELECT * FROM KYStatisUsers_BST.statis_', cur_GameParameter, v_month, '_users WHERE StatisDate=''', in_StatisDate, ''''); 
		END LOOP; 
	CLOSE cur1;	

	-- 添加所有游戏当天数据
	IF NOT EXISTS(SELECT * FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'KYStatisUsers_BST' AND TABLE_NAME = CONCAT('statis_allgames',v_month,'_users')) THEN
		CALL KYStatisUsers_BST.sp_createStatisticsUsersTable(CONCAT('statis_allgames',v_month,'_users'),0); 
	END IF; 
	SET @v_sqlselect = CONCAT(IF(LENGTH(v_sqlbase) > 0,CONCAT('REPLACE INTO KYStatisUsers_BST.statis_allgames',v_month,'_users (StatisDate,Account,ChannelID,LineCode,CellScore,WinGold,LostGold,Revenue,WinNum,LostNum,currency,exchangeRate) 
	...
	-- 添加执行日志
	INSERT INTO KYStatisUsers_BST.event_log_record (sp_name, starttime, endtime, exectime,mark) VALUES ('sp_StatisUsersDayData', v_starttime, v_endtime, TIMESTAMPDIFF(SECOND,v_starttime,v_endtime),in_StatisDate); 
END;
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
...
BEGIN
	...
	IF NOT EXISTS(SELECT * FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'KYStatisUsers_BST' AND TABLE_NAME = CONCAT('statis_allgames',v_month,'_users')) THEN
		CALL KYStatisUsers_BST.sp_createStatisticsUsersTable(CONCAT('statis_allgames',v_month,'_users'),0); 
	END IF; 
	SET @v_sqlselect = CONCAT(IF(LENGTH(v_sqlbase) > 0,CONCAT('REPLACE INTO KYStatisUsers_BST.statis_allgames',v_month,'_users (StatisDate,Account,ChannelID,LineCode,AllBet,CellScore,WinGold,LostGold,Revenue,WinNum,LostNum) 
	...
	-- 添加执行日志
	INSERT INTO KYStatisUsers_BST.event_log_record (sp_name, starttime, endtime, exectime,mark) VALUES ('sp_StatisUsersDayData_allbet', v_starttime, v_endtime, TIMESTAMPDIFF(SECOND,v_starttime,v_endtime),in_StatisDate); 
END;
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
...
BEGIN
    ...
    IF in_StatisDate IS NULL THEN
        SET in_StatisDate = DATE_ADD(CURDATE(),INTERVAL -1 DAY); 
    END IF; 
    select in_StatisDate AS in_StatisDate; 
--     SET start_Time = CONVERT_TZ(CONCAT(in_StatisDate,' 00:00:00'), 'Europe/London','Asia/Shanghai'); 
--     SET end_Time = CONVERT_TZ(CONCAT(in_StatisDate,' 23:59:59'), 'Europe/London','Asia/Shanghai'); 
-- +0 时区
    SET start_Time = DATE_ADD(CONCAT(in_StatisDate,' 00:00:00'),  INTERVAL 8 HOUR); 
    SET end_Time = DATE_ADD(CONCAT(in_StatisDate,' 23:59:59'),  INTERVAL 8 HOUR); 
    SET v_month=DATE_FORMAT(in_StatisDate,'%Y%m'); 
    ...
    IF NOT EXISTS(SELECT * FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'KYStatisUsers_BST' AND TABLE_NAME = CONCAT('statis_',in_GameName,v_month,'_users')) THEN
    ...
    IF EXISTS(SELECT * FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'detail_record' AND TABLE_NAME = v_tblname) THEN
        SET v_sqlbase=CONCAT('select ''',in_StatisDate,''' AS StatisDate, Accounts,ChannelID,LineCode,
                    ...
                    currency
					FROM ','detail_record.',v_tblname,' FORCE index(index_gameendtime)  WHERE GameEndTime>=''',start_Time,''' AND GameEndTime<=''',end_Time,'''
					GROUP BY Accounts,ChannelID,LineCode,currency'); 
    END IF; 

    IF(LENGTH(v_sqlbase) > 0) THEN
        SET v_sqlbase = CONCAT('REPLACE INTO KYStatisUsers_BST.statis_',in_GameName,v_month,'_users (StatisDate,Account,ChannelID,LineCode,AllBet,CellScore,WinGold,LostGold,Revenue,WinNum,LostNum,currency,exchangeRate)
...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP EVENT IF EXISTS `event_StatisUsersDayData_BST_08:30`;
CREATE DEFINER=`root`@`%` EVENT `event_StatisUsersDayData_BST_08:30` ON SCHEDULE EVERY 1 DAY STARTS '2023-01-17 08:30:00' ON COMPLETION NOT PRESERVE DISABLE ON SLAVE DO call `KYStatisUsers_BST`.`sp_StatisUsersDayData`(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
