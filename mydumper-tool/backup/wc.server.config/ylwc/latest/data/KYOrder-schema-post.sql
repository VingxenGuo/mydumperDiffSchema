...
SET character_set_results = utf8mb4;
SET collation_connection = utf8mb4_general_ci;
DROP PROCEDURE IF EXISTS `sp_cashOrderDetail_test`;
CREATE DEFINER=`root`@`%` PROCEDURE `sp_cashOrderDetail_test`(
IN pPlayerAccount VARCHAR(200)
,IN pGameUserNO VARCHAR(50)
,IN isEmptySearchDate INT
,IN pAgentId INT
...
'); 
    ...
    IF IFNULL(pLimit,0) > 0 AND IFNULL(pOffset,-1)>-1 THEN
        ...
        SET tWalletPlayerHistorySql = CONCAT(tWalletPlayerHistorySql,' LIMIT ',pLimit,' OFFSET ',pOffset,' '); 
