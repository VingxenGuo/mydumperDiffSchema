...
BEGIN
	...
	IF LENGTH(v_sqlbase) <> 0 THEN		
		...
		ELSE
			...
			SET v_sqlselect = CONCAT('SELECT * FROM all_game_record LIMIT 1000'); 
...
CREATE DEFINER=`root`@`%` EVENT `event_statisAgentRecordInfo_BST09:30` ON SCHEDULE EVERY 1 DAY STARTS '2023-01-17 09:30:00' ON COMPLETION NOT PRESERVE ENABLE DO call `KYDB_NEW`.`sp_statisAgentRecordInfo_BST_everyMonth`(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_statisAgentRecordInfo_reguser02:00` ON SCHEDULE EVERY 1 DAY STARTS '2018-05-22 02:00:00' ON COMPLETION NOT PRESERVE ENABLE DO call sp_statisAgentRecordInfo_everyMonth_reguser(null);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
