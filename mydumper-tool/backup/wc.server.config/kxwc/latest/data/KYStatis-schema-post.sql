...
CREATE DEFINER=`root`@`%` EVENT `event_sp_statis_room_recordtype_data` ON SCHEDULE EVERY 30 MINUTE STARTS '2021-03-09 19:05:00' ON COMPLETION NOT PRESERVE ENABLE DO call sp_statis_room_recordtype_data(CURDATE(),CURDATE());
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_statisSameTableUsers_every20min` ON SCHEDULE EVERY 20 MINUTE STARTS '2018-05-31 23:50:00' ON COMPLETION NOT PRESERVE DISABLE DO CALL sp_statisSameTableUsers(CURDATE());
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
SET @PREV_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT;
...
CREATE DEFINER=`root`@`%` EVENT `event_statis_game_daily_everyday01:00` ON SCHEDULE EVERY 1 DAY STARTS '2024-07-20 01:00:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL KYStatis.sp_statis_game_daily_data(NULL,NULL);
SET character_set_client = @PREV_CHARACTER_SET_CLIENT;
SET character_set_results = @PREV_CHARACTER_SET_RESULTS;
SET collation_connection = @PREV_COLLATION_CONNECTION;
